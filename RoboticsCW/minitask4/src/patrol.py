...
class CameraController(): 
    def __init__(self): 
        ...
        # Create a list of waypoints (x,y) from the map in RViz 
        self.waypoints = [(-0.12, -1.5), (2.28,0.3), (4.37,-1.91), (1.03,1.83)] 
        self.current_waypoint_index = 0 #Index of the current waypoint the robot is navigating to 
        # Creates an instance for interacting with the move_base action server 
        self.move_base_client = actionlib.SimpleActionClient('move_base', MoveBaseAction) 
        # self.is_patrolling = True # Patrol flag to change state 
    ...
    def patrol(self): 
        #Checks if there are more waypoints to visit 
        while self.current_waypoint_index < len(self.waypoints):  
            ...
            rospy.loginfo(f"Moving to waypoitn {self.current_waypoint_index + 1}: ({x}, {y})") 
            ...
            if self.move_base_client.get_state() == GoalStatus.SUCCEEDED: 
                ...
                rospy.logwarn(f"Failed to reach waypoint {self.current_waypoint_index + 1}") 
            

        else: 
            #Reached the end of the waypoints, reset the index 
            rospy.loginfo("Finished route.") 
    ...
    def vision(self):
        ...
        while not rospy.is_shutdown():
            ...
            elif self.state == RobotStates.PATROL: 
                self.patrol() 
